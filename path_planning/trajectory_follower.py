...
class PurePursuit(Node):
    ...
    def pose_callback(self, odometry_msg):
        ...
        error = perpendicular[closest_segment_index]
        output_error = Float32()
        output_error.data = error
        self.error_pub.publish(output_error)
        ...
        for i in range(closest_segment_index, len(starts)):
            ...
            # self.lookahead = np.sqrt(np.linalg.norm(ends[i] - robot_point)) + 0.5 
            break
        ...
        # self.lookahead = np.pi - 5*np.abs(delta) 
        if l1 < 0.05:
            self.speed = 0.0
